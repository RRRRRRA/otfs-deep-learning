...
def train_model_cnn_ce_mse(train_loader, test_loader, epochs = 50, lr = 1e-4, optimizer_name = 'adam',
    ...
    for epoch in range(epochs):
        ...
        if loss_test_mean < min_test_loss:
            ...
            s = BASE_PATH + 'model_weights/chanest_cnn_ce_mse_{}_{}_{}_{}_{}.pth'.format(lr, optimizer_name, dropout_rate, l1_weight, l2_weight)
            torch.save(model.state_dict(), s)

        if verbose:
            print('-' * 80)

    return (loss_train_means, loss_train_ce_means, loss_train_mse_means), (loss_test_means, loss_test_ce_means, loss_test_mse_means)
...
try:
    f = open(BASE_PATH + 'results_maps/results_map_chanest_cnn_ce_mse.pkl', 'rb')
    results_map_chanest_cnn_ce_mse = pkl.load(f)
    f.close()
except:
    results_map_chanest_cnn_ce_mse = {}
...
for lr in lrs:
    for optimizer_name in optimizer_names:
        for dropout_rate in dropout_rates:
            dataset_train_cnn_ce_mse = ChannelEstimationDatasetCNNCEMSE(otfs_data, train_indices)
            train_loader_cnn_ce_mse = DataLoader(dataset_train_cnn_ce_mse, batch_size = BATCH_SIZE, shuffle=True)
            ...
            print(k)
            if k in results_map_chanest_cnn_ce_mse:
                print(k, 'already trained, skipping')
                continue
            results_map_chanest_cnn_ce_mse[k] = train_model_cnn_ce_mse(train_loader_cnn_ce_mse, test_loader_cnn_ce_mse, epochs = EPOCHS,
                                                lr = lr, optimizer_name = optimizer_name, dropout_rate = dropout_rate, verbose = True)


            f = open(BASE_PATH + 'results_maps/results_map_chanest_cnn_ce_mse.pkl', 'wb')
            pkl.dump(results_map_chanest_cnn_ce_mse, f)
            f.close()
